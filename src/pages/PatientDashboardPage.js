const { waitForPageLoad, dismissDialogs } = require('../helpers/utils');
...
class PatientDashboardPage {
  ...
  async goToTasks() { await this.navigateToTab('Tasks'); }
  async goToMDOrders() { await this.navigateToTab('MD Orders'); }
  async goToCommunication() { await this.navigateToTab('Communication'); }
  async goToAdverseEvents() { await this.navigateToTab('Adverse Events'); }
  async goToMedication() { await this.navigateToTab('Medication'); }
  async goToMiscellaneous() { await this.navigateToTab('Miscellaneous'); }
  async goToNursingCarePlan() { await this.navigateToTab('Nursing Care Plan'); }
  async goToChart() { await this.navigateToTab('Chart'); }
  async goToVSMonitor() { await this.navigateToTab('VS Monitor'); }

  // Go to patient dashboard via URL
  async goto(patientId, episodeId) {
